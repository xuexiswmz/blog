...
export async function GET(request: Request) {
  try {
    ...
    const comments = rows.map((row) => ({
      ...
      moderationSource: row.moderation_source
        ? String(row.moderation_source)
        : null,
      moderationReason: row.moderation_reason
        ? String(row.moderation_reason)
        : null,
      moderationModel: row.moderation_model
        ? String(row.moderation_model)
        : null,
...
export async function PATCH(request: Request) {
  try {
    ...
    const humanReason =
      action === "approve" ? "管理员人工通过" : "管理员人工拒绝";
    ...
    return Response.json({
      ...
      comment: {
        ...
        moderationSource: String(comment.moderation_source),
        moderationReason: String(comment.moderation_reason),
        moderatedAt: new Date(String(comment.moderated_at)).toISOString(),
      },
    });
  } catch (error) {
