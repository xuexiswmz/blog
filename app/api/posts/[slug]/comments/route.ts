        ...
        const comments: CommentDto[] = 
            rows.map((row)=>{
                ...
                return {
                    ...
                    replyToUsername: row.reply_to_username && !replyTargetDeleted ? String(row.reply_to_username) : null,
                    replyToContent: row.reply_to_content && !replyTargetDeleted ? String(row.reply_to_content) : null,
                    deleted,
                    replies: []
                }
            })
            ...
            // 获取所有根评论
            const rootComments = comments.filter((comments)=>comments.rootId===null)
            const rootsById = new Map(rootComments.map((comment)=>[
                comment.id,
                comment
            ]))
            ...
            const count = rootComments.reduce((total, comment)=> total + (comment.deleted ? 0 : 1)+ 
...
export async function POST(
    ...
    try {
        ...
        const username = typeof body.username === "string" ? body.username.replace(/\s+/g, " ").trim() : ""
        const content = typeof body.content === "string" ? body.content.replace(/\s+/g, " ").trim() : ""
        const replyToId = body.replyToId === undefined || body.replyToId === null || body.replyToId === "" ? null : typeof body.replyToId === "string" ? body.replyToId : "invalid"
